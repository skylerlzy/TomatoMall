...
function handleTabChange(tab: any) {
  if (tab.paneName === 'specifications' && (!productForm.value.title || !productForm.value.price || !productForm.value.rate || !productForm.value.description || !productForm.value.cover || !productForm.value.detail || !productForm.value.author || !productForm.value.tag)) {
...
function handleUpload(file: any) {
  productForm.value.cover = file.url  // 修正此处，应该赋值给cover而不是title
  let formData = new FormData()
  formData.append('file', file.raw)
  uploadImage(formData).then(res => {
    productForm.value.cover = res.data.data  // 修正此处，应该赋值给cover并使用data而不是result
  })
}
