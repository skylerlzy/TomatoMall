...
const calculateTotalPrice = () => {
  ...
  if (selectedCoupon.value && originalPrice.value >= selectedCoupon.value.need) {
    totalPrice.value = originalPrice.value - selectedCoupon.value.volume;
  } else {
    ...
    if (res.data.code === '200') {
      ...
      if (selectedCoupon.value) {
        updateOrderPrice(
          ...
          router.push(`/payForm/${orderId.value}`);
        }).catch(() => {
          ElMessage.error('更新订单价格失败');
        });
      } else {
        router.push(`/payForm/${orderId.value}`);
      }
        ...
        <div class="order-summary">
          <div class="price-detail">
            ...
            <div class="price-item" v-if="selectedCoupon">
              <span>优惠金额</span>
              <span class="discount">-¥{{ selectedCoupon.volume.toFixed(2) }}</span>
            </div>
          </div>
