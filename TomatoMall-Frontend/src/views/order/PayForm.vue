...
import { orderPay, getOrderStatus } from "../../api/order";
...
const fetchPaymentForm = async () => {
  try {
    const res = await orderPay(orderId);
    if (res.data.code === '200') {
      const { paymentForm: form, ...details } = res.data.data;
      ...
      if (res.data.code === '200') {
        ...
        if (status === 'PAID') {
          ...
          if (cartRes.data.code === '200') {
            ...
            // 逐个删除购物车商品
            const deletePromises = cartItems.map(item => deleteCart(item.id));
...
<template>
  <div class="payment-container">
    <div v-if="isPaymentFormVisible" class="payment-info">
      ...
      <p>支付金额：￥{{ paymentDetails.totalAmount }}</p>
      <p>支付方式：{{ paymentDetails.paymentMethod }}</p>
    </div>
