...
// 钱包数据
const balance = ref<number>(0) // 用户余额
const rechargeAmount = ref<number>(100.0) // 默认充值金额
const paymentMethod = ref<string>('Alipay') // 默认支付方式
const transactions = ref<Transaction[]>([]) // 交易历史记录
...
const fetchWalletInfo = async () => {
  ...
  setTimeout(() => {
    ...
    transactions.value = [
      { id: 1, type: '充值', amount: 200.0, date: '2023-11-15 14:30:22', status: '成功' },
      { id: 2, type: '消费', amount: -99.0, date: '2023-11-12 09:15:36', status: '成功' },
      { id: 3, type: '充值', amount: 500.0, date: '2023-11-01 18:22:10', status: '成功' },
...
// 处理充值
// 修改调用 checkoutWallet 的方式，直接传递两个参数
const handleRecharge = async () => {
  ...
  try {
    // 确保金额为正确的数值类型，并精确到两位小数
    const amount = parseFloat(rechargeAmount.value.toFixed(2));

    // 调用 API，直接传递两个参数
    const res = await checkoutWallet(amount, paymentMethod.value);
    ...
    if (res.data.code === '200') {
      ...
      // 延迟跳转到支付页面
      setTimeout(() => {
        router.push(`/walletPayForm/${orderId}`);
      }, 1000);
    } else {
